...
public class CosmeticBandana extends CosmeticBase {
   private final CosmeticBandana.ModelTopHat modelTopHat;
   private static final ResourceLocation TEXTURE = new ResourceLocation("EsdeathClient/shi.png");

   public CosmeticBandana(RenderPlayer var1) {
      super(var1, "Bandana");
      this.modelTopHat = new CosmeticBandana.ModelTopHat(var1);
   }
   ...
   public void render(AbstractClientPlayer var1, float var2, float var3, float var4, float var5, float var6, float var7, float var8) {
      if ((CosmeticController.shouldRenderTopHat(var1))) {
         ...
         CosmeticController.apply("Bandana");
         ...
         if (var1.isSneaking()) {
            GL11.glTranslatef(0.0F, 0.23125F, 0.0F);
         }
   ...
   private class ModelTopHat extends CosmeticModelBase {
      ...
      public void render(Entity var1, float var2, float var3, float var4, float var5, float var6, float var7) {
         if ((EsdeathUtils.isOnline(var1.getName()))) {
            if ((EsdeathClient.getInstance().getPlayer(var1.getUniqueID().toString()).getCosmetics().contains("bandana"))) {
